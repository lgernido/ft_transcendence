...
document.addEventListener('DOMContentLoaded', () => {
		...
		async function moveBall() {
			...
			if (posBallX - 1.5 <= 0 || posBallX + 1.5 >= 100)
			{
				...
				updateScore();
				if (scrorePlayerLeft >= limitPoint || scrorePlayerRight >= limitPoint)
				{
					console.log("Stop game");
					clearInterval(ballInterval);
					winner = scrorePlayerLeft >= limitPoint ? document.getElementById('playerLeft').textContent : document.getElementById('playerRight').textContent;
